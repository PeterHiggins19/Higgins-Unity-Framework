...
Three things. First, the monitoring architecture — continuous compositional drift detection with governance. Second, EITT — the geometric mean preserves Shannon entropy under temporal decimation. Third, the d(CoDa)/dt chain — perturbation velocity gives the scalar speed, the ILR balance trajectory gives the structural path, and the balance derivative dB/dt gives the directed rate of structural change along each partition. The architecture is engineering. EITT is an empirical finding we can't prove. The d(CoDa)/dt chain connects them — the derivative describes the compositional motion, EITT describes what is conserved when you compress the time axis of that motion. And in raw proportions, the zero-sum constraint (Σ dx_i/dt = 0) IS the relay chain — every gain financed by losses elsewhere. In ILR coordinates, the balances move independently, which is why ILR is the right space for the temporal derivative.
