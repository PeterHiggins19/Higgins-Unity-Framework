...
| **E-03** | **Zero replacement artifact** | NO — HUF works with raw proportions, zeros are valid (carrier absent = 0). | YES — log(0) is undefined. Every CoDa pipeline must replace zeros before log-ratio transforms. Replacement method (multiplicative, Bayesian, count-zero) affects results. | NEW AT UNION — HUF previously tolerated zeros naturally. CoDa integration forces zero replacement, inserting instrument-generated values into the data. A carrier that was genuinely absent now has a phantom presence. | Yes: vary replacement method (multiplicative vs Bayesian vs count-zero). If signal changes, the replacement is the source. | Partially: report sensitivity to replacement method. Flag any drift signal that depends on zero-replacement choice. |
...
**"Here are 12 ways our instruments degrade. Here's which ones I brought to you, which ones you'd bring to me, and which ones only appear when we work together. Here's how to test for each one. Here's how to govern each one. The entanglement makes us stronger — but only if we're honest about what it costs."**
