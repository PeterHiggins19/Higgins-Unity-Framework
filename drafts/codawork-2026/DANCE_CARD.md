...
**The d(CoDa)/dt connection:** He asked in his initial reply: "how do you describe changes along time?" The answer lives in three layers already built into the tools. Perturbation velocity — d_A(x(t), x(t−1)) — is the scalar speed of compositional change. The ILR balance trajectory B(t) is the accumulated structural path in his coordinate system. The missing middle layer is dB/dt — the instantaneous rate of structural change along each balance partition. When dB₁/dt is negative and steep, fossil is losing ground to renewable — that IS the energy transition expressed as a derivative in the coordinates he invented. EITT then answers the complementary question: does this motion's information content survive temporal compression? The derivative describes the motion. EITT describes what is conserved. And the zero-sum constraint in raw proportions (Σ dx_i/dt = 0) is not a limitation — it IS the relay chain. Every gain is financed by losses elsewhere. In ILR space, the balances move independently — each dB/dt is free — which is why ILR is the correct coordinate system for taking the temporal derivative.

**Why he'll care:** His poster is on the Aitchison norm as a concentration measure. K_eff is exp(Shannon entropy). You just showed his geometric mean preserves the thing his norm measures. And the d(CoDa)/dt framing puts the temporal question in his own ILR coordinates — not foreign territory.
...
**What you bring:** "If compositional time series are functional data on the simplex, EITT gives you a principled decimation operator. The geometric mean is the correct coarsening filter — it preserves information content. And we have a three-layer temporal chain: perturbation velocity gives the scalar speed, the balance trajectory gives the structural path, and the balance derivative dB/dt gives the directed rate of change along each partition. In functional CoDa terms, d(CoDa)/dt taken in ILR coordinates is the natural velocity field on the simplex."

**What you ask:** "Would your functional CoDa framework predict EITT's invariance, or is it a surprise? And does the balance derivative dB/dt — the first derivative of the ILR trajectory — fit into your functional analysis as a well-defined velocity on the simplex?"

**Why d(CoDa)/dt matters here:** Hron's functional CoDa treats compositions as continuous curves. The derivative of those curves is exactly dB/dt. If he already has the mathematical machinery for functional derivatives on the simplex, then dB/dt is native to his framework — and EITT tells him what is conserved when he coarsens those curves. The decimation operator and the derivative operator are linked: one compresses time, the other differentiates along it.
...
Four things. First, the monitoring architecture — continuous compositional drift detection with governance. Second, EITT — the geometric mean preserves Shannon entropy under temporal decimation, now confirmed across five domains (energy, hardware, financial, cosmological, commodities). Third, the d(CoDa)/dt chain — perturbation velocity gives the scalar speed, the ILR balance trajectory gives the structural path, and the balance derivative dB/dt gives the directed rate of structural change along each partition. Fourth, the EITT inversion principle — when EITT fails on a K-part system, increasing K until it holds reveals the system's true dimensionality and hidden state variables. The gold/silver ratio fails at K=2 over 338 years but holds at K=4 when volatility and momentum carriers are recovered. The failure is the signal.
...
No longer true. As of April 9, 2026, EITT has been tested on five domains: energy (electricity generation, 7 countries), hardware degradation (Backblaze drive stats, K=4, 24 months — 0.03%), financial markets (equity sector shares, K=9, 74 months — 0.08%), cosmological observation (Planck 353 GHz half-mission split — 0.3%), and commodities (gold/silver ratio, K=2, 338 years — fails globally at 6.7%, holds within regimes at 0.5% and under K=4 reconstruction at 0.4%). The boundary condition is temporal autocorrelation. We publish our code and data for all five domains.
...
- "The balance derivative tells you how fast and in which direction. EITT tells you what survives compression."
- "The zero-sum constraint is not a limitation — it IS the relay chain."
...
4. **"The balance derivative dB/dt — the rate of structural change in ILR coordinates — describes the compositional motion. EITT describes what survives when you compress the time record of that motion. The derivative and the invariant are complementary views of the same temporal structure."**
...
| **Finance** | Do sector shares have compositional structure? | 9 sectors, 74mo: EITT holds at 0.08%. COVID relay visible. | Sector rotation IS a compositional relay. |
| **Cosmology** | Does polarization composition survive temporal splitting? | Planck 353 GHz half-mission: EITT holds at 0.3%. | Spatial EITT fails — diagnoses sky heterogeneity. |
...
> "I came here with a monitoring protocol, an empirical result I can't prove, and 7 adversarial failures I'm showing you on purpose. You have the geometry. I have the application. The geometric mean — your geometric mean — preserves information across time in ways nobody has formalized. The balance derivative dB/dt in your ILR coordinates tells us how fast and in which direction the composition is moving. EITT tells us that the information content of that motion survives temporal compression. The derivative and the invariant are two views of the same temporal structure — and neither existed in the literature before this year. If we can prove it together, the next conversation is with Ramsar, because there are 2,500 wetlands that need compositional monitoring and nobody is doing it yet. We can't prove it. Can you?"
...
| Financial (sector shares) | 9 | Temporal | 74 months | 0.0797% | HOLDS |
...
| Gold/Silver (K=4 reconstructed) | 4 | Temporal | 338 years | 0.3844% | HOLDS |
| Planck CMB (spatial) | 2 | Spatial | 4096:1 | 18.117% | FAILS* |

*Spatial failure is diagnostic: quantifies sky heterogeneity.

**The EITT Inversion Principle:** When EITT fails on K parts, increase K until it holds. The number of carriers needed IS the system's true dimensionality. The carriers you had to add ARE the hidden state variables. For gold/silver: K_true = 4 = gold + silver + volatility + momentum. The monetary regime state lives in the hidden carriers.
