...
We are also exploring a temporal derivative — the rate of compositional change in ILR coordinates. Perturbation velocity (scalar speed), balance trajectory (structural path), and balance derivative dB/dt (directed rate) can be computed for any compositional time series. This is promising but exploratory work, not yet independently validated.
...
We are also exploring whether the balance derivative dB/dt — the rate of structural change along each ILR partition — could make the gap operational: not just detecting drift, but measuring which partitions are moving and how fast. This is implemented in the tools but not yet validated beyond the tested domains.
...
This is not a claim that EITT holds across all these domains. As of April 9, 2026, it has been tested on five domains: energy (holds), hardware degradation (Backblaze K=4, holds at 0.03%), a 120-stock price-level portfolio (holds at 0.08% — note: price-level weighted, not market-cap), cosmological polarization (Planck half-mission temporal split holds at 0.3% — note: spatial decimation claim retracted due to HEALPix NESTED ordering issue), and commodities (gold/silver K=2 fails at 6.7%, K=4 with recovered hidden carriers holds at 0.38%, validated out-of-sample). Bootstrap confidence intervals computed for all domains. Each domain listed above remains an open question until tested.
...
We found the signal in energy, then confirmed it in hardware and a financial price-level portfolio. We tested it on cosmological data (temporal split holds, spatial claim retracted). We mapped the boundary. We showed where it breaks — and showed that the breaks reveal hidden structure. We ran the adversarial audit and fixed what was wrong. We cannot prove the invariance analytically.
