...
**The zero-sum constraint:** In raw proportions, Σ dx_i/dt = 0 always (because Σ x_i = 1). This is a mathematical identity of closure, not an empirical finding. The relay pattern (which carriers gain and which lose) is the empirical observation.

**Why ILR (conjectural):** In ILR coordinates, the balance derivatives are free — each dBₖ/dt moves independently. No zero-sum constraint. If temporal derivatives prove useful for CoDa, ILR may be the natural coordinate system for them.

**Possible connection to EITT (exploratory):** The balance derivative describes the compositional motion. EITT may describe what is conserved when you compress the time record. This connection is conjectural — we have not established it formally.
...
**EITT spatial extension (conceptual — not yet validated):**
Replace temporal decimation with spatial coarse-graining (geometric mean of spatially adjacent compositions). **Note:** The Planck spatial test was incorrectly implemented (HEALPix NESTED ordering does not preserve spatial locality). Proper spatial EITT requires spatially coherent decimation (e.g., HEALPix ud_grade or RING ordering). The concept remains open but has no valid empirical test yet.
