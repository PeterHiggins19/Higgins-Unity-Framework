...
This is not a claim that EITT holds across all these domains. As of April 9, 2026, it has been tested on five domains: energy (holds), hardware degradation (holds at 0.03%), financial sector shares (holds at 0.08%), cosmological polarization (holds at 0.3% on temporal split), and commodities (fails at K=2, holds at K=4 with recovered hidden carriers). Each domain listed above remains an open question until tested.
...
We found the signal in energy, then confirmed it in hardware, finance, and cosmology. We mapped the boundary. We showed where it breaks — and showed that the breaks reveal hidden structure. We cannot prove the invariance analytically.
