...
- **Cross-Domain Confirmation (April 9, 2026).** EITT tested on five domains: energy (holds), hardware degradation (Backblaze K=4, 0.03%), financial sector shares (K=9, 0.08%), Planck CMB polarization (half-mission split, 0.3%), and gold/silver commodities (K=2 fails at 6.7%, K=4 with recovered hidden carriers holds at 0.38%). The EITT inversion principle: failure diagnoses missing compositional dimensions. Results in [`code/analysis/`](code/analysis/).
