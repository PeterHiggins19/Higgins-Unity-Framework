#!/usr/bin/env node
// ══════════════════════════════════════════════════════════════════════
// HUF Collective Trace v5.4 + HUF-Org — Builder
// Comprehensive documentation with Car/Fuel Analogy as the foundation
// Peter Higgins · March 2026
// ML-HUF Structural Identity validation: Learning organisms, multiple substrates
// ══════════════════════════════════════════════════════════════════════
...
function titlePage() {
  return [
    ...
    SP(),
    P([{ text: 'Collective Trace Report v5.4', font: 'Times New Roman', size: 28, bold: true, color: MID }], { align: AlignmentType.CENTER }),
...
function section1CarAnalogy() {
  return [
    ...
    H2('Adaptive Scope'),
    P('Every system is a system of systems. Claude is a system. Claude and the operator form a system. The operator and their projects form a larger system. All systems nest within systems at every scale. HUF works when all elements within the defined scope are fully represented. If elements are missing, adjust the scope — narrow the center frequency and bandwidth of observation until the data present matches the portfolio.'),
    P('Dynamic OCC drift monitoring means the 51/49 operator/tool ratio is not static — it drifts, and MC-4 watches that drift. The ratio portfolio adapts as the system evolves. The fuel gauge monitors the drift in real time, alerting when the declared ratio deviates from the actual balance.'),
    P('Dynamic portfolio gating means elements enter and leave the active portfolio based on observability, like a noise gate in audio or a softmax gate in neural networks. An element is included when it is observable and removes from the portfolio when it becomes unobservable. Unity is always computed within the gated set. As data arrives or departs, the boundary adapts. The gate responds automatically to reality.'),

    H2('HUF-Org: The Living System'),
    P('HUF is not a framework imposed on systems. HUF IS a system itself — a living organism that grows, integrates, and self-regulates. The unity constraint Σ=1.0 is a metabolic law: the organism allocates a finite energy budget across all functional organs. Every element shares this budget. Growth is healthy only when portfolio rebalancing occurs simultaneously.'),
    P('Cancer is uncontrolled growth without rebalancing. A tumor\'s share rises while other elements maintain their claims on the budget. The total demand exceeds available energy. The organism fails. HUF prevents cancer through mandatory portfolio rebalancing: every new element triggers adjustment across ALL existing elements. Deceptive Drift is detected and rejected before it becomes pathological.'),
    P('Integration follows a viability protocol: propose a new element, run a Monte Carlo test (perturb its share, observe all existing elements, check Q-sensitivity bounds), measure the outcome, and decide: accept, reduce rate, or reject. New elements join one at a time. Integration rate is governed by the most Q-sensitive (fragile) element. This is biological wisdom: slow, tested, reversible growth.'),
    P('MC-4 is the immune system. It detects drift (antigens), rejects incompatible integrations, and signals stress. Before integration, viability testing acts like an antigen test: is this new element compatible? If the test fails, integration is rejected. If it passes, the organism grows safely. The organism survives because growth is cautious, tested, and self-correcting.'),
    SP(),
    makeHufOrgTable(),
    SP(),

    H2('Machine Learning as HUF-Org'),
    P('Machine learning systems exhibit identical structure to biological organisms and acoustic systems under HUF-Org. A neural network is a living system governed by the unity constraint. Weight space is a ratio portfolio: all weights sum through softmax to exactly 1.0. The learning rate governs integration speed — how quickly weights adjust to new data. Overfitting is cancer: the model learns training data perfectly while failing at unseen data, exactly like a loudspeaker over-corrected for a single microphone position. Regularization (dropout, L1, L2) is the immune system preventing pathological growth. Early stopping is the viability test: when validation loss stops improving, the organism has reached carrying capacity and must stop integrating.'),
    P('Batch size scales the observational bandwidth. A batch size of 1 is a single measurement point (high variance, high specificity). A batch size equal to the full dataset is the ensemble average (low variance, high generalization). Dropout in neural networks literally averages across random subsets of neurons, analogous to acoustic measurement averaging across microphone positions. The training loss curve is logistic growth: initial epochs yield steep improvements, later epochs yield diminishing returns, and the system approaches a carrying capacity. Push beyond and overfitting claims the model.'),
    P('This parallel validates HUF across substrates. Biological tissue (metabolic budgets), acoustic transducers (diffraction budgets), and silicon neural networks (parameter budgets) share identical mathematics under Σ=1.0. The substrate changes. The unity constraint remains.'),
    SP(),
    makeMLHUFTable(),
    SP(),
...
function section2ProjectState() {
  return [
    ...
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
      children: [new TextRun({ text: '• Sufficiency Frontier v3.5 — Defines the boundary between sustainable and collapsed systems', font: 'Times New Roman', size: 22, color: DK })] }),
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
      children: [new TextRun({ text: '• Fourth Category v2.5 — Establishes the mathematical framework beyond traditional triads', font: 'Times New Roman', size: 22, color: DK })] }),
    ...
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
      children: [new TextRun({ text: '• Vol 8 Synthesis v1.4 — Integration of all Phase 1 concepts', font: 'Times New Roman', size: 22, color: DK })] }),
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
    ...
    H2('Rogue Wave Audio v2.5'),
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
      children: [new TextRun({ text: '• Organic Digital Loudspeakers — 17 comprehensive sections (with 17.6 ML parallel), 11 detailed tables, 25 scientific references', font: 'Times New Roman', size: 22, color: DK })] }),
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
...
function section5RepoArchitecture() {
  return [
    ...
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
      children: [new TextRun({ text: '• Organic Digital Loudspeakers paper (v2.5, 17 sections with ML parallel)', font: 'Times New Roman', size: 22, color: DK })] }),
    new Paragraph({ spacing: { after: 160 }, indent: { left: 720 },
...
async function build() {
    ...
    sections: [{
      ...
      headers: { default: new Header({ children: [
        new Paragraph({ alignment: AlignmentType.CENTER,
          ...
          spacing: { after: 0 },
          children: [new TextRun({ text: 'HUF Collective Trace — v5.3', font: 'Times New Roman', size: 18, color: MID, italics: true })] })
      ] }) },
  ...
  const out = '/sessions/wonderful-elegant-pascal/mnt/Claude CoWorker/HUFv4/HUF_Collective_Trace_v5.4.docx';
