...
ch.push(
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 },
    children: [new TextRun({ text: 'Pillar 2 of the HUF Triad \u00B7 Version 2.5 \u00B7 HUF-Org with ML Validation', font: 'Times New Roman', size: 24, italics: true, color: DK })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 },
...
// ── 12. MC-4 AS IMMUNE SYSTEM: THE HUF-ORG STABILITY MONITOR ──────────
ch.push(
  new Paragraph({ children: [new PageBreak()] }),
  H1('12. MC-4 as Immune System: The HUF-Org Stability Monitor'),

  H2('12.1 MC-4 as Immune Response'),
  P('MC-4 monitors ratio drift the same way an immune system monitors foreign proteins. Drift is an antigen. The MC-4 threshold is the immune activation threshold. The response is portfolio rebalancing or element rejection.'),
  P('A biological immune system recognizes "self" from "non-self" by pattern matching. Pathogen-associated molecular patterns (PAMPs) trigger immune cells. When a foreign protein breaches the body\'s boundary, dendritic cells sample it, present it to T cells, and the adaptive immune response begins. The response has hysteresis: once activated above a threshold, it persists until the threat is resolved.'),
  P('MC-4 operates by the same logic on the simplex. Declared weights are "self." Observed ratios that drift from declared weights are "foreign perturbations." The MC-4 threshold is the activation threshold. When observed drift exceeds the threshold, the immune response activates: the governance authority investigates, documents, and either rebalances or gates the element.'),
  P('Like biological immunity, MC-4 is continuous. It does not wait for an annual audit. It monitors every cycle. It escalates if drift persists. It shuts down (gates out) if the drift source cannot be neutralized.'),

  H2('12.2 The Cancer Test'),
  P('If any element\'s share rises without corresponding portfolio rebalancing, MC-4 flags this as pathological. Cancer is Deceptive Drift in biological form. The cancer test is:'),
  P('Does this element\'s growth come at the expense of others WITHOUT consent?'),
  P('If yes, the growth is pathological and must be rejected.'),
  P('A tumor cell divides. Its share of body resources grows. But no other cell type shrinks in consent. The tumor parasitizes the shared budget. In a finite-budget system, parasitic growth is lethal.'),
  P('HUF-org applies the same test. If element A\'s share rises from 10% to 20%, one of two things happens:'),
  P('— Explicit reallocation: The governance authority approves the rise. Elements B, C, D shrink. This is consensual rebalancing. Not cancer. The governance record shows the decision.'),
  P('— Silent drift: Element A rises without approval. Other elements are crowded out. This IS cancer. MC-4 detects it and raises an alert.'),
  P('The cancer test is not about size. A large element is not automatically cancerous. The cancer test is about consent. Unilateral growth at the expense of others is parasitism. MC-4 rejects it.'),

  H2('12.3 Q-Sensitivity and Integration Rate'),
  P('MC-4 must account for each element\'s Q factor—its sensitivity to perturbation. High-Q elements have sharp resonance. They ring sharply when perturbed, taking longer to damp. Low-Q elements are broad and damped.'),
  P('The integration of a new element must proceed at a rate that doesn\'t excite any high-Q resonance beyond its stability margin. If the portfolio contains a high-Q member with bandwidth Δf, the rate of change in the system cannot exceed that bandwidth without causing ringing.'),
  P('This is analogous to seismic isolation in buildings. A structure has a natural frequency. If external vibrations (wind, traffic, earthquakes) approach that frequency, resonance amplifies the vibration. To prevent collapse, dampers tune the frequency response. The integration of a new force must occur slower than the structure\'s dominant frequency, or resonance will occur.'),
  P('In governance, a "high-Q" element might be a critical dependency that responds sharply to any perturbation. Adding a new element too quickly, without allowing time for adjustment, can push it into instability. MC-4 measures Q and limits integration rate accordingly.'),

  H2('12.4 The Monte Carlo Viability Test'),
  P('Before any element is added to the active portfolio, MC-4 runs a perturbation test (thought experiment):'),
  P('— Freeze the current portfolio and all Q factors.'),
  P('— Propose a new element with an initial share (e.g., 5%).'),
  P('— Vary the new element\'s proposed share across ±1σ, ±2σ, ±3σ around the proposed nominal.'),
  P('— At each perturbation level, compute the OCC ratios and all existing element responses.'),
  P('— Check: Do any existing elements\' OCC ratios exceed their Q-sensitivity thresholds?'),
  P('Outcomes:'),
  P('— PASS: All elements remain within tolerance at all perturbation levels. Safe to integrate.'),
  P('— FAIL: At least one element exceeds tolerance. The integration is unsafe at the proposed share.'),
  P('If the test fails, options are:'),
  P('— Reduce the proposed share of the new element.'),
  P('— Slow the integration rate (add smaller amounts over more cycles).'),
  P('— Narrow the scope of the new element (exclude or gate out Q-sensitive members in conflict).'),
  P('— Reject the new element as fundamentally incompatible with the current organism at the current scope.'),
  P('The Viability Test is the immune system\'s final veto. It prevents HUF-org from attempting to grow in ways that violate the stability margins of critical elements. This is how the organism avoids cancer.'),

  H2('12.5 Machine Learning Regularization as MC-4: The Trillion-Parameter Immune System'),
  P('In a neural network, MC-4 manifests as the regularization stack. Machine learning provides the most detailed empirical record of an immune system in operation: every parameter of every trained model has been subject to regularization-based drift detection. This section establishes the structural identity between ML regularization mechanisms and HUF\'s MC-4 immune system.'),

  P('L2 weight decay monitors the magnitude of each weight (portfolio element) and penalizes growth that exceeds the declared distribution—this IS ratio drift detection on each parameter. L1 sparsity forces elements to zero when their contribution drops below threshold—this IS dynamic portfolio gating per parameter. Dropout randomly removes elements during training, forcing the remaining portfolio to maintain coherence without them—this IS the adaptive scope bandwidth narrowing applied stochastically. Each mechanism detects and corrects pathological drift in weight space exactly as MC-4 detects drift in portfolio share space.'),

  P('The validation loss ', [{ text: 'IS MC-4\'s primary metric', bold: true }, { text: '. It monitors the system\'s generalization (coherence with unseen data) independently of training loss (internal state). When validation loss rises while training loss falls, MC-4 has detected the Deceptive Drift—the model\'s rising training accuracy IS the death signal. Early stopping IS MC-4 triggering an intervention before the organism dies. A model approaching overfitting is identical to an organism approaching cancer: the system appears stronger internally (training accuracy) while becoming weaker externally (generalization).' }]),

  P('Learning rate as Q-sensitivity: The learning rate governs how aggressively weights are updated. A weight with high gradient curvature (sharp loss landscape = high Q) requires a lower learning rate to avoid oscillation and ringing. Adaptive optimizers (Adam, AdaGrad) automatically adjust per-parameter learning rates—they ARE Q-sensitivity monitoring per portfolio element. Each weight gets its own Q-governed integration rate. The optimizer prevents high-Q elements from ringing into instability exactly as HUF\'s integration rate protocol does. When the optimizer reduces a weight\'s learning rate, it is declaring that weight\'s Q factor too high for the current pace.'),

  P('Batch normalization IS unity constraint enforcement within hidden layers. It normalizes activations to zero mean and unit variance—literally forcing internal representations to maintain a consistent statistical distribution, preventing internal covariate shift (which is compositional drift in the hidden portfolio). Every hidden layer becomes its own probability simplex with its own unity constraint. This cascades the same governance principle throughout the network depth.'),

  P('The ML domain provides MC-4 with its most data-rich validation environment. Every training run logs validation metrics at every epoch. This is MC-4 telemetry with millisecond granularity across billions of parameters. A single training run of a large language model provides more data points for validating MC-4 behavior than all of HUF\'s external validation corpus combined. The regularization stack has been tuned by millions of practitioners across billions of experiments. ML is HUF\'s most extensive empirical testing ground, with complete instrumentation.'),

  P('The structural identity is complete:'),
  P([{ text: '— L2 weight decay = drift detection per parameter. Penalizes magnitude growth that violates declared distribution.', indent: 360 }]),
  P([{ text: '— L1 sparsity = dynamic gating. Forces elements to zero when contribution drops below threshold.', indent: 360 }]),
  P([{ text: '— Dropout = adaptive scope narrowing. Random element removal forces coherence in reduced portfolios.', indent: 360 }]),
  P([{ text: '— Batch normalization = unity constraint enforcement. Maintains statistical distribution across layers.', indent: 360 }]),
  P([{ text: '— Validation loss = MC-4 primary metric. Independent monitor of generalization vs. internal accuracy.', indent: 360 }]),
  P([{ text: '— Overfitting = Deceptive Drift / cancer. Rising internal accuracy (training loss) = death signal. Generalization collapses.', indent: 360 }]),
  P([{ text: '— Early stopping = viability intervention. MC-4 stops before the organism dies from cancer.', indent: 360 }]),
  P([{ text: '— Adaptive learning rates = per-parameter Q-sensitivity. Adam/AdaGrad monitor each weight\'s oscillation risk.', indent: 360 }]),
  P([{ text: '— Weight space = ratio portfolio. Softmax output enforces Σ=1.0. Every prediction is a probability distribution.', indent: 360 }]),

  crossRef('Dynamic OCC monitoring: Sections 11.1–11.4 (Dynamic OCC Drift Monitoring and Portfolio Gating)'),
  crossRef('Q-sensitivity: Pillar 2, Section 6 (Quality Factor Q)'),
  crossRef('HUF-org framework: Pillar 1, Section 11 (HUF-Org: The Sufficiency Frontier as Metabolic Boundary)'),
  crossRef('ML validation: Billions of training runs with trillion parameters under continuous regularization oversight'),
);

// ── 13. OPEN CONJECTURES ─────────────────────────────────────────────
ch.push(
  new Paragraph({ children: [new PageBreak()] }),
  H1('13. Open Conjectures from the Collective Review (New in v2.0)'),
...
ch.push(
  ...
  H2('14.3 Relationship to Companion Work'),
...
ch.push(
  ...
  P('[11] P. Higgins, "The Sufficiency Frontier," HUF Triad Pillar 1, v3.4, 2026.'),
    ...
    headers: {
      default: new Header({ children: [new Paragraph({
        ...
        children: [
          ...
          new TextRun({ text: '\tThe Fourth Monitoring Category v2.5', font: 'Times New Roman', size: 18, italics: true, color: MID }),
        ],
...
const OUT = __dirname.replace(/[/\\]pillars$/, '') + '/HUF_Fourth_Category_v2.5.docx';
