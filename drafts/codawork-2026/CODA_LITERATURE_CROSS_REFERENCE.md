...
The d(CoDa)/dt chain deepens this concern. Three temporal layers exist: perturbation velocity (scalar speed, |d(CoDa)/dt| on the simplex), balance trajectory B(t) = ilr(x(t)) (structural path), and balance derivative dB/dt (directed rate of structural change along each ILR partition). Aliasing corrupts all three differently — velocity may appear low when the signal oscillates between samples, balance trajectories appear smoother than they are, and balance derivatives lose high-frequency structural dynamics. The zero-sum constraint in raw proportions (Σ dx_i/dt = 0) forces the relay chain — every gain financed by losses elsewhere — and ILR coordinates decompose that constrained relay into independent balance derivatives. EITT connects here: smooth balance trajectories (small |dB/dt|) predict high temporal persistence, which predicts EITT holding. Noisy/aliased trajectories predict failure.

**New concern for CoDa:** Compositional Nyquist. The control chart literature assumes the sampling interval is appropriate, but provides no test for whether it is. HUF's velocity metric provides a natural test: if velocity doubles when interval halves, the original interval was insufficient. The balance derivative dB/dt provides a sharper test: if the sign of dB/dt changes when you halve the interval, the original interval was aliasing a structural oscillation into apparent monotonic drift.
...
The offer: HUF brings the application context that reveals which CoDa concerns matter most in practice, and surfaces new ones that static analysis will never encounter. CoDa brings the mathematical rigor that eliminates errors HUF couldn't even detect on its own. The d(CoDa)/dt chain — perturbation velocity as scalar speed, balance trajectory as structural path, balance derivative dB/dt as directed rate of structural change — provides the temporal analysis framework that connects EITT (what is conserved under temporal compression) to the dynamics themselves (how fast and in which direction the composition is moving). The zero-sum constraint in raw proportions forces the relay structure; ILR coordinates decompose it into independent structural rates. This temporal layer is what CoDa's spatial geometry has not yet developed — and it arises naturally from continuous monitoring.
