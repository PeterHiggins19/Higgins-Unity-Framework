...
In the energy datasets we tested, Shannon entropy appeared empirically near-invariant under geometric-mean temporal decimation. We measured 0.18% variation across 341:1 compression, confirmed on 6 countries (1.02% mean), then drove the residual to 0.029% on a third independent test. We mapped a boundary condition (temporal autocorrelation required), linked the residual to a second-order Jensen correction (the Hessian footprint), and adversarial-tested the result (10 pass, 7 fail). Cross-domain tests on four additional domains produced three confirmations, one informative failure, and several honest caveats (see honesty notes in README).
...
| d(CoDa)/dt | (Exploratory) Temporal derivative of compositional data — perturbation velocity (speed), balance trajectory (path), balance derivative dB/dt (directed structural rate). Promising but not yet independently validated. |
| Zero-sum relay | In raw proportions, Σ dx_i/dt = 0 — every gain financed by losses elsewhere. A mathematical identity of closure, not an empirical finding. |
...
These complement HUF's original metrics (TV distance, K_eff complexity, peak carrier change) to provide dual-metric monitoring — both information-theoretic and simplex-native measures on the same data. The balance derivative dB/dt is exploratory — it produces interpretable outputs but has not been formally validated or connected to EITT analytically.
