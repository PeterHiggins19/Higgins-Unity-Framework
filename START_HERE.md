...
Shannon entropy is empirically near-invariant under geometric-mean temporal decimation. The geometric mean — the fundamental operation of CoDa — appears to preserve information content across temporal resolutions. We measured 0.18% variation across 341:1 compression, confirmed on 6 countries (1.02% mean), then driven to 0.029% on a third independent test. We found the boundary condition (temporal autocorrelation required), mapped the residual to a second-order Jensen correction (Hessian footprint), and adversarial-tested it (10 pass, 7 fail — failures all at the boundary).

We found this empirically. We can't prove it. We think you can.
